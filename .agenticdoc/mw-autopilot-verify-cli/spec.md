...
- **AC-007（机器级层 + 优先级真值表）** 机器层路径解析顺序同 RAG（`MW_<X>_FILE` → `MW_<X>_HOME` → `HOME` → `USERPROFILE`，缺失即层空且**不建目录**）；优先级 `cli > project > machine > default`，逐字段 origin 可断言；机器层未知字段的行为**显式定义**（建议：只告警不作废——RAG 的"整份作废"会让一个 typo poison 所有项目）；缓存键必须包含机器层的 `(path, mtime_ns, size)`，否则机器层改动不生效。 **[REVISED @ 2026-09-26]** ① 删除 `cli` 层（`set` 落盘即 project；一次性 flag 到不了独立 conductor 进程）；② “缓存键必须包含机器层的 `(path, mtime_ns, size)`”句**作废**——`cached_load` 的 stat 短路已删除（无缓存）；③ 机器层覆盖域由“xkey 三键 + cwd”**收窄为 `xkey_verify_cmd` + `xkey_verify_cwd` 两键**，规则 = **空值即未决定**（`[]` / `""` ⇒ 用机器层；非空 ⇒ 项目胜出；两层均空/缺失 ⇒ `default`）。依据：两侧写者（`save_config` 与 TS `saveConfig`，后者有明确不变量 "saves normalize the file to the full canonical set"）都会把 13 键**材料化**写进项目文件，故“键存在即胜出”会被材料化出的空值永久挡住机器层（U-1 失效）；`xkey_repair`（功能开关，机器层硬开会造成项目无法关闭）与 `xkey_verify_timeout_s`（材料化后无法区分“显式默认 1800”与“未配置”）退出机器层域。用户 2026-09-26 确认选项 1。
