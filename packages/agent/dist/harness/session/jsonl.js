...
function invalidFile(path, line, message, cause) {
...
function parseMutation(line, path, lineNumber) {
    ...
    switch (value.kind) {
        case "entry": {
            ...
            return {
                kind: "entry",
                lane,
                entry: { ...entryFields, id, type, parentId, seq, timestamp },
            };
        }
        case "record": {
            const id = requireString(value.id, path, lineNumber, "id");
...
class JsonlSessionStorage {
    ...
    createLane(lane, at) {
        return this.enqueue(async () => {
            ...
            const mutation = { kind: "lane", seq: this.sequence + 1, lane, leafId: at };
    ...
    moveLane(lane, to) {
        return this.enqueue(async () => {
            ...
            const mutation = { kind: "lane", seq: this.sequence + 1, lane, leafId: to };
            await this.appendMutation(mutation);
            this.applyMutation(mutation);
        });
    }
    ...
    appendForkLane(lane, leafId) {
        return this.enqueue(async () => {
            this.validateTarget(leafId);
            const mutation = { kind: "lane", seq: this.sequence + 1, lane, leafId };
            await this.appendMutation(mutation);
            this.applyMutation(mutation);
        });
    }
    ...
    async findRecords(query = {}) {
        ...
        if (query.afterSeq !== undefined)
            this.validateCursor(query.afterSeq);
        const results = [];
    ...
    async getLog(options = {}) {
        ...
        if (options.afterSeq !== undefined)
            this.validateCursor(options.afterSeq);
        const results = [];
    ...
    setName(name) {
        return this.enqueue(async () => {
            const mutation = { kind: "fact", seq: this.sequence + 1, fact: "name", name };
            await this.appendMutation(mutation);
            this.applyMutation(mutation);
        });
    }
    ...
    setLabel(id, label) {
        return this.enqueue(async () => {
            ...
            const mutation = { kind: "fact", seq: this.sequence + 1, fact: "label", targetId: id, label };
            await this.appendMutation(mutation);
            this.applyMutation(mutation);
        });
    }
    ...
    applyMutation(mutation, path = this.metadata.path, line = this.sequence + 2, validateEntryChain = true) {
        ...
        if (seq !== this.sequence + 1)
            throw invalidFile(path, line, `has non-consecutive seq ${seq}`);
        switch (mutation.kind) {
            case "entry": {
                ...
                if (validateEntryChain) {
                    ...
                    if (mutation.entry.parentId !== leafId)
                        throw invalidFile(path, line, "does not chain to the lane leaf");
                }
    ...
    requireLaneForReplay(lane, path, line) {
        const leafId = this.lanes.get(lane);
        if (leafId === undefined)
            throw invalidFile(path, line, `references missing lane ${lane}`);
        return leafId;
    }
    ...
    validateUnusedIdForReplay(id, path, line) {
        if (this.usedIds.has(id))
            throw invalidFile(path, line, `contains duplicate id ${id}`);
    }
...
export class JsonlSessionRepo {
    ...
    create(options = {}) {
        return this.enqueue(async () => {
            ...
            fileResult(await this.fs.createDir(await this.root(), { recursive: true }), `Failed to create sessions directory`);
    ...
    list() {
        return this.enqueue(async () => {
            ...
            if (!fileResult(await this.fs.exists(root), `Failed to check sessions directory ${root}`))
                return [];
            const files = fileResult(await this.fs.listDir(root), `Failed to list sessions directory ${root}`).filter((entry) => entry.kind !== "directory" && entry.name.endsWith(".jsonl"));
            const metadata = [];
    ...
    fork(source, options = {}) {
        return this.enqueue(async () => {
            ...
            if (name !== undefined)
                await target.setName(name);
            for (const entry of copiedEntries) {
                const label = await sourceSession.getLabel(entry.id);
                if (label !== undefined)
                    await target.setLabel(entry.id, label);
            }
            return target;
        });
    }
    ...
    enqueue(operation) {
        if (this.disposed)
            return Promise.reject(new SessionError("storage", "JSONL session repository is disposed"));
        const result = this.tail.then(operation);
        this.tail = result.then(() => undefined, () => undefined);
        return result;
    }
    async openDirect(metadata) {
        const existing = this.storages.get(metadata.path);
        if (existing)
            return new Session(existing);
        if (!fileResult(await this.fs.exists(metadata.path), `Failed to check session ${metadata.path}`)) {
    ...
    async pathForId(id) {
        ...
        try {
            ...
            throw new SessionError("invalid_payload", `Invalid session id ${JSON.stringify(id)}`, error instanceof Error ? error : undefined);
        }
        return fileResult(await this.fs.joinPath([await this.root(), `session-${encoded}.jsonl`]), `Failed to resolve path for session ${id}`);
    }
}
