...
export function createSessionBackendConformance(factory) {
    return [
        ...
        createCase(factory, "records and log", "distinguishes zero one and multiple open operations", async (repository) => {
            const session = await repository.create({ id: "session" });
        ...
        createCase(factory, "records and log", "does not let an earlier finish close a later operation start", async (repository) => {
            const session = await repository.create({ id: "session" });
            await session.appendRecord({
                type: "operation_finished",
                id: "early-finish",
        ...
        createCase(factory, "records and log", "scopes open operations by lane kind and limit", async (repository) => {
            ...
            const mainCompaction = await session.appendRecord(operationStarted("main-compaction", { lane: "main", kind: "compaction" }));
