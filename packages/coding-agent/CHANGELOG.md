...
- Added the `xkey-authorize` gate kind and the cross-key repair config keys to the extension-side mirror (xkey-repair-mechanism): `GATE_KINDS` in `autopilot/status-model.ts` gains the new kind alongside the Python closed set, and `AutopilotConfig`/`DEFAULT_CONFIG`/`BOOL_FIELDS`/`INT_RANGES` gain `xkey_repair` (default `false`), `xkey_verify_cmd` (default `[]`, validated as a list of non-empty strings) and `xkey_verify_timeout_s` (default `1800`, `>= 1`), field-for-field identical to `autopilot/config.py`. Required rather than optional: both sides reject unknown config fields fail-closed, so a project that opted in would otherwise have its config reported invalid and the key dropped. The default object is now handed out through a fresh copy, since the list default would otherwise be shared and mutable across readers.

- Added the cross-window xkey gate-dir guard (`shared/xkey-gate-guard.ts`, xkey-repair-mechanism): a `tool_call` hard block, registered in every mode next to the protected-config guard, that denies agent `write`/`edit`/`bash` attempts to touch anything under `<project>/.agenticdoc/_autopilot/gates/` - the human-answer channel, whose files `gates.py` treats as the source of truth and whose manual edit counts as a legal answer. Without it an agent could answer its own authorization gate through its own tool layer with no observable refusal, and `answered_by` is only a self-reported `hostname:pid` (a human keystroke and a PM agent share the same pid and filesystem permissions, so there is no cryptographic discriminator to build on). Deliberately narrow: reads pass through, and the whole `xkey/` tree below the gate dir (`evidence/**` proposals, `ledger.json`, `tickets/**`) stays writable, so the guard can never kill the proposal path it depends on. A blocked call appends an `[XKEY_GATE]` line to the task trace.log when running as a worker.

- Added the RAG configuration guide for the extension-side config reader (mw-rag-config-guide): `packages/multi-workers/docs/rag-config-guide.md` documents every field `rag/config.ts` accepts (the server table, the `rag:` section, roles/phases - `roles` also accepts `chat_budget`/`time_budget_s` - and `rag-roots.json`) with copy-paste examples and the merge/origin semantics, and `mw rag init` generates the commented template it quotes. `test_rag_docs.py` byte-pins the manual's three template blocks to `mw rag init --print` and asserts the documented field set equals the validator's, so the TS-accepted field names cannot silently drift out of the docs.

- Added the parallel-first protocol to PM windows (mw-parallel-protocol): `pmActivate` registers a `before_agent_start` handler that appends a constant rule block to the run's system prompt �?analyze the parallel units, the shared-resource/file conflict surface, and the serial reasons before decomposing any phase; spec/design research and evidence collection default to one fully parallel batch of `type: research` workers (read-only allowlist, so the only conflict surface is the output file) with exactly one unique `evidence/research/<phase>-<rq-slug>-<date>.md` per research question; coding splits by file/module boundary with one worker per file at a time; phase docs (spec.md/design.md) stay PM-serial. Append-only, marker-guarded idempotent, and static text so the system-prompt prefix stays cacheable and no session tokens accumulate (an injected message would add one per run). PM-only by construction: worker windows run `workerModeActivate` and never receive the block. Takes effect after a bundle rebuild (`mw build --install`) plus a window restart.

- Added configurable RAG integration to agent-team-loop (mw-rag-integration): RAG servers are declared in two layers (machine `~/.agents/rag-servers.yml` and project `<control>/.mw/rag-servers.yml`, field-level merge with a per-field `origin` table) and selected per project through the `target.yml` `rag:` section (`enabled`, `default_server`, `roles`, `phases`, `budgets`), so projects that do not enable RAG are untouched: no tools registered, no task.md block, no probe requests, no files written. An enabled project gets six worker tools (`rag_search`, `rag_symbol`, `rag_graph`, `rag_impact`, `rag_sources`, `rag_feedback`) with a `server` enum from the resolved config, plus `rag_chat` registered only for the new `rag-research` task type (600s per-call cap). Tool results are normalized into one envelope whose `citation` grammar is `server:source:file_path:line` (leftmost two segments are server/source, the rightmost numeric segment is the line, everything between - including `::` - is the file path) with `local_path`/`exists`/`line_hint`/`snapshot_warning` resolved inline against the configured `path_roots`. Degradation is layered: unreachable at session start = registered but annotated `[unreachable at session start]`, runtime failures trip a circuit breaker (3 consecutive connect/timeout/protocol errors; capability errors excluded), `both` transport falls back mcp->cli only for read-only tools on connect-level failures (`rag_feedback`/`rag_chat`/timeouts never auto-fallback). Per-call and cumulative budgets (`rag_chat_budget:`/`rag_time_budget_s:` task.md headers, default 900s, wall-clock 70% guard) plus a 30s heartbeat keep long retrieval and chat calls observable. Every call appends evidence lines to the worker trace (`rag_call` with the resolved wire tool name, `rag_fallback`, `rag-required-missing`, `rag-rewrite-degraded`, `rag-unavailable`), and `role.require OR phase.require` (with the task.md `phase:` header written by the dispatchers) produces a `rag-required-missing` line plus an output.md marker when a required task ends without a verifiable citation (warning-only in this release). The `overcode-v1` adapter owns the logical-name to wire-name mapping (`rag_graph`->`graph_query`, `rag_sources`->`list_sources`+`list_collections` merged into one envelope, rewrite-enabled `rag_search`->`rag_search_multi_rounds`) as a data table, so a second server family is a new adapter rather than a config-schema change.

- Added the implementation entry gate (mw-implementation-gate): a `tool_call` hard gate in agent-team-loop that blocks `write`/`edit` (and narrowed `bash` write-target) calls hitting repo code paths (`packages/**` code extensions, excluding node_modules/dist/.tmp) when the window has no active AgenticTask key claim — enforcing the framework discipline mechanically instead of relying on the SKILL.md/AGENTS.md text alone. A gated write passes on any of: an active `_index.parallel` claim matching this window's host:pid, a dispatched worker environment (`PI_WORKER_TASK`), or a fresh (≤24h) `.agenticdoc/<key>/mini-spec.md` (trivial-fix fast path; audited). Blocks return an actionable reason naming both compliant paths; every block and mini-pass appends an audit line to `.agenticdoc/_impl_gate.log` (best-effort, never affects the decision). `bash` coverage is target-side only (redirect targets and tee/cp/mv/rm/sed -i operands) and explicitly not a sandbox (inline `python -c` writes, pre-written scripts, `find -delete` are known, documented misses); target lexing follows real shell escape semantics, so Windows backslash path spellings (`> H:\\repo\\packages\\x.py`, unquoted or quoted) resolve correctly instead of collapsing to `H:repopackagesx.py`. Takes effect after `mw setup --build` + window restart; windows running old bundles stay ungated until restarted.

- Added `zai-coding-cn` to the dispatch model prefix namespace (mw-provider-routing): `PROVIDER_ID_TO_PREFIX` maps the provider to the `zai/` prefix (model values like `zai/glm-5.3`), mirrored on the Python side (`mw_common.MODEL_PREFIX_TO_PI_PROVIDER`) and locked by the parity tests on both sides. The PM window records a zai session into `.mw/window-model` and dispatches resolve through the launcher's new direct zai-coding-cn route (no mw proxy, credential env `ZAI_CODING_CN_API_KEY`).
...
- Added an explicit task type and a reason-gated model override to the worker dispatch surface (mw-dispatch-role-escape): `dispatch_worker` and `/worker` accept `type: coding|review|research`. Previously the type was derived from the cli alone, so a pi worker could never reach the `review`/`research` dispatch.yml role or its tool allowlist - the gap that forced PMs into hand-written `model:` overrides. Both entries share one frontmatter planner and one echo line naming the resolved type, role, and model layer (`dispatch.yml <role>=<v>` / `role default <v> -> <x> (reason: ...)`).
...
- Added an autopilot progress section to the `/autopilot monitor` panel and auto-show for enabled projects (mw-autopilot-stall-feedback): the panel now renders tick freshness (`tick seq N (3s ago)`, `STALE (conductor not ticking)` past `max(30s, 5 x poll_interval)`), slot usage (`slots 1/2`), pending-gate identity per key, and one row per key that needs attention — phase (`_index.parallel`) / roadmap key-status / in-flight workers / dependency blockers — with a stalled key highlighted next to its advance failure run (`advance 12x interface-drift 2m ago "unknown phase …"`) and the recovery command (`-> /autopilot gate gate-0002 approve|reject`). The derivation (`deriveAutopilotPanel`, `deriveAdvanceStalls`, `readTimelineTail`) is read-only, injectable for tests, degrades to placeholders for missing/corrupt sources, and reads the timeline through a bounded tail window instead of replaying the whole file. The stall run clears only on a success of the *same* edge, matching `conductor._advance_failure_streak` (a neighbouring boundary's success neither breaks nor joins the run), so the panel never reports recovery while the guard inside the conductor keeps counting. `autoStartMonitor` is called from the PM `session_start` hook (the first place a UI context exists) so enabled projects get the panel without running a command; `/autopilot monitor off` suppresses it for the session and `monitor on` clears that. The config mirror gained `advance_stall_ticks` (default 5) — required because both sides validate unknown fields fail-closed.

- Added framework-written checkpoint evidence and a narrow worker file tool to agent-team-loop (mw-worker-progress-persist): the checkpoint every worker gets at `min(30m, budget/2)` now writes its machine line (`CKPT <n>m [machine] ts=<ISO> reads=<n> writes=<n> phases=<d>/<t>|-> repeat_top=<k> risk=<low|mid|high>`) into `<taskDir>/progress.md` from the framework itself, so a read-only role no longer needs the ability to write a file for the checkpoint to leave evidence behind - the failure mode behind OverCode's `chroma-review-evidence-r1` (146 tool calls, `reads=141 writes=0`, final reply asking the PM to record its conclusions). Roles whose resolved allowlist has no `write` (review/verifier/reviewer/research/rag-research) additionally get a basename-only `worker_file` tool registered (`progress.md`, `report.md`, `report-<slug>.md`/`report.<slug>.md`, ≤ 64 KiB, `append`/`write`), and their checkpoint steer no longer instructs an impossible append to `progress.md`: it points at `worker_file` or at a one-line reply instead, while the coding-role steer text stays byte-identical. Rejections throw, so they land in trace.log as `[TOOL_ERR]` next to the accepted call's `[TOOL]` (which now also carries the target file name), and no path parsing happens at all, so `..`/drive/UNC spellings, `task.md`, `trace.log`, `output.md` and `worker.log` are unreachable by construction. `writes` still counts only real `write`/`edit` calls (the narrow tool is deliberately not a write tool, so a zero-write read-only worker stays visible), and `activeToolsForType` is the single place the extra tool is added - `TOOL_ALLOWLISTS`/`toolsForType` and the Python dispatch REGISTRY stay unchanged.
- Added phase-layered docs gating to the worker dispatch surface (mw-worker-visibility-gate): the gate now reads the owning key's phase (`pm-state.md`, the same source the task.md `phase:` header uses) and requires only what that phase implies. At `SPEC` (also `init`/unset, and any unrecognized value) it checks the spec side alone - `spec.md` >= 500 bytes, >= 1 `evidence/research/spec-*.md`, a non-empty `## §0 Goal Alignment` section carrying 预期收益, >= 1 numbered `AC-NNN` - and only from `DESIGN` onward does it also demand `design.md` >= 500 bytes plus a `design-*` note. Both dispatch entry points (`dispatch_worker` and the `/worker` command) share one `dispatchDocGaps(root, key, phase?)`; the new parameter is optional and defaults to the spec tier, so neither entry can drift. Refusal text and `DOC_GATE_HINT` are unchanged and no bypass flag was added, and task type is deliberately not gated (a `type: coding` dispatch at SPEC stays allowed, since reviews and research are dispatched as coding to get a file-landing channel - see P-007).

- Added the cross-key hint line to the bottom watch widget (mw-worker-visibility-gate): tasks this window dispatched into another key (an explicit `dispatch_worker(key: ...)` or the `_scratch` fallback) now render as exactly one trailing line - `~ N elsewhere: <owner>(<counts>; risk=high:K)` - instead of being invisible. The panel keeps listing only the watched key's rows (a merged list would misattribute another key's work), the line is truncated at the existing 110-character cap, owners are ordered by row count then name so the text is stable, and `risk=high:K` counts cross-key *running* rows whose `progress.md` checkpoint is high risk. `renderWatchLines` takes the dispatched-key set as an optional sixth argument, so every existing caller (12 test call sites included) renders byte-identically when it is absent.

- Added `pm-state.md` claim mirroring to agent-team-loop (mw-worker-visibility-gate): after a successful TS claim or takeover (`switch_key`, `/pm-key new|switch`, and the session-restore quiet re-claim) `syncPmStateClaimId` rewrites the `- Claim-Id:` line in `<key>/pm-state.md` to the same `host:pid` the `_index.parallel` row holds. The two stores previously had writers with different pid semantics (the framework's `update_index.py claim` stores the short-lived python process's pid, the TS takeover rewrote only the index row), so a key's pm-state silently disagreed with the row every consumer actually reads (`implementation-gate` liveness and claim ownership). The write is a single-line in-place replacement that detects the file's dominant EOL (every `pm-state.md` in this repo is CRLF), writes atomically through `tmp`+rename, and never creates or re-templates the file: a missing file or missing interface line returns a reason, the claim still succeeds because the index row is authoritative, and the tool text or notification carries one warning line. `StateManager.write()` is deliberately not reused - it rewrites the old three-section template and would erase the seven-section body.
- Added cross-key divergence escalation to agent-team-loop (mw-crosskey-risk-escalation): the monitor's risk escalation now also fires for workers this window dispatched into another key (or `_scratch`) instead of only the watched key's own rows; the alert names the owner key in that case, low-risk and once-per-task semantics are unchanged, and workers dispatched by other windows never wake this one.

- Added the extension-side mirror of the xkey config surface and its write lock (mw-autopilot-verify-cli): `AutopilotConfig`/`DEFAULT_CONFIG`/`STR_FIELDS` gain `xkey_verify_cwd` (default `""`, the 13th key, field-for-field identical to `autopilot/config.py`), `BOOL_FIELDS`/`LIST_FIELDS`/`INT_RANGES` are exported so the cross-language test compares the registries directly, and `configLockPath(root)`/`saveConfigLocked(root, cfg, opts)` wrap the same `<root>/.mw/autopilot-config.lock` the Python side uses, with the four console config handlers taking that lock for their read-modify-write.

- Added the TypeScript half of the cross-language config parity suite (mw-autopilot-verify-cli): one shared corpus of 53 payloads drives both implementations, asserting agreed verdicts for every payload, byte-identical canonical writes for the 31 accepted ones, idempotent round trips in both directions, and a one-key file that stays one key in the raw view while resolving to 13 keys in the parsed view on both sides. The Python interpreter is required rather than skippable, so the differential cannot silently degrade into a tautology, and the corpus hash and case count are asserted against the frozen values.

### Changed

- Scoped the worker-task tool surface to this window (mw-task-scope-isolation): `list_tasks` defaults to `scope: "mine"` (the watched key plus tasks this window dispatched in this process) instead of listing the whole project, gains `scope: "key"` (with `key`) and `scope: "all"` (the previous whole-project view), and every row now carries its owning key (`<owner> :: <taskKey> | <status> | ...`) so a cross-key list stays readable. An unknown scope, or `scope: "key"` without `key`, returns a text error instead of silently falling back to the project view. `ack_worker_result` and `/mw ack` are scoped the same way: `all` covers only rows this window owns, and naming another key's row is refused with the owning key plus a `/pm-key switch <owner>` hint instead of writing the project-level `.agenticdoc/_workers.acked` sidecar (previously any window could ack - and silently clear - another window's unhandled rows). Cross-key work stays ackable where it belongs: an explicit `dispatch_worker(key: ...)` is recorded on the window's watch state, so a task dispatched into another key still shows up in `mine`. The dispatch record is process-local (never persisted, never shared).
...
- Changed `readConfig` to reject numeric literals that are not integers (mw-autopilot-verify-cli): an `int` field written as `4.0`, `4.00`, `1e2`, `1E2` or `-0.0` was rounded by `JSON.parse` and accepted, while the Python validator rejected the same file, so the two sides disagreed about it. The reviver inspects the literal through the parse context and keeps the narrow type (no `any`); whole-valued literals such as `4` and `2**53` stay accepted and byte-identical on both sides.
...
- Added the worker model badge to the bottom watch widget rows (agent-team-loop): every row shows the model it runs as `[id]` after the task key — running/terminal rows read the pi-resolved id from a new `[MODEL]` trace.log line (written by worker-mode at session start; launcher defaults like glm-5.3 included), pending rows fall back to the dispatch-time `--model` override from the queue row. Old traces without `[MODEL]` keep rendering without the badge.
- Added the pi `shellPath` row to `/mw doctor` output (agent-team-loop): the new Python-side `pi_shell` section renders as `pi shell: <path>` when configured, `pi shell: <status> — <detail>` for missing/broken/unreadable states, and stays silent on non-Windows (not-applicable) and for serves predating the section.
- Added the `advance_phase` tool (agent-team-loop PM mode): a shell-free path through the AgenticTask phase gates. `advance_phase.py` could previously only be run by the PM agent through its bash tool, and the pm-state guard blocks hand-editing `- Phase:` as the alternative — on fresh machines where pi's shell resolution or the in-shell `python` differs (WSL-only bash, Store-stub python, python3-only Linux) the PM deadlocked with "no shell, cannot execute advance_phase.py / update_index.py". The tool spawns the framework script directly (`.claude/scripts` with the skill clone as fallback, list args, no shell, combined stdout+stderr surfaced), keeping gate semantics in the Python script; the guard hint now points at the tool first.
- Added `/pm-save [note]` (agent-team-loop): persist this window's working context into the watched key's `pm-state.md` so a restarted pi window (e.g. after a framework rebuild) can pick the work back up. The command writes a mechanical session snapshot (watch key, window claim, `_index.parallel` row, worker counts, free note) into the Notes area — machine-interface lines (`- Phase:` / `- Claim-Id:`) stay byte-identical and prior notes survive — then explicitly asks the agent to fill in the conversational state (current thread of work, decisions, next steps). On restart, `restoreWatch` notices the saved file (pointing the user at it, never injecting it): reading the Notes area resumes the context. Requires a watched key (`/pm-key switch` or `/mw-watch`).
...
- Added `/mw target show | set --game <dir> [--engine <dir>] [--vcs git|p4|none] [--uproject <file>] | clear` (agent-team-loop): dual-workspace config from the pi window — a thin wrapper over `mw.py target` (Python stays the single source of parsing/validation/rendering), with double-quote-aware path parsing for paths containing spaces. `set`/`clear` remind that dual mode applies on the next worker spawn (no serve restart needed).
- Added dual-workspace support to `agent-team-loop` workers: `.agenticdoc/target.yml` (`mode: dual` with `game`/`engine` roots, `env MW_TARGET_GAME/MW_TARGET_ENGINE` overrides) moves the worker cwd to the game root while every coordination write (trace.log / output.md / phase docs / goal check) stays anchored to the control workspace via the `PI_WORKER_TASK` derivation — the target tree receives zero framework files. Includes `shared/target-config.ts` (config parsing with toolchain/ignore/contract sections, `{game}/{engine}/{uproject}` placeholder rendering, unique-`.uproject` discovery, fail-closed validation).
- Added `deny_globs` to worker task.md frontmatter: a context firewall enforced at the tool-call layer (read/ls/find/grep) ahead of read-scope allow rules, matched dual-basis (normalized absolute path and game-root-relative path, minimatch) with deny-wins precedence; a trailing `/**` glob also denies the bare directory itself (plain minimatch would let `ls`/`find` on the directory list its contents). `deny_globs` without `read_scope` is a valid deny-only mode; neither field keeps zero interception.
- Added workspace-profile injection at TS dispatch time (`pm/task-dispatcher.ts`): `dispatchTask` augments a task.md with the target.yml essentials (resolved toolchain commands, ignore firewall, contract with `docs:` references) plus the control-root absolute path, and injects `deny_globs` frontmatter in the Python renderer's exact format; re-dispatch is idempotent, task-level `deny_globs` is never overwritten, and an unusable target.yml refuses the dispatch (per-task isolation in the scan loop, self-healing on the next scan once the config is fixed).
- Added `controlRootFromTaskPath()` to `agent-team-loop/shared/paths.ts`: explicit 5-level derivation of the control workspace root from a worker task path (the write-anchor contract the dual-root execution relies on).
- Added the agent-team-loop cross-window protected-config guard (`shared/protected-config.ts`): a hard tool-call block, registered in every mode (PM, worker, interactive), that denies write/edit/bash attempts to modify the shared pi agent config (`~/.pi/agent/auth.json`, `models.json`, `settings.json`, `oauth.json`, and the agent dir itself) while sessions run. Motivated by the 2026-09-15 incident: a running session cleared `auth.json` and every live window lost its credentials ("Provider is not configured") because those files hot-reload per window. Reads pass through; bash matching expands `~`/`$HOME`/`%USERPROFILE%`/`$PI_CODING_AGENT_DIR` spellings and fails closed on write constructs (rm/mv/cp, PowerShell cmdlets, `sed -i`, `find -delete`, redirects into the protected paths, inline `python -c`/`node -e` writes); worker-mode blocks append a `[PROTECTED_CONFIG]` line to the task trace.log for the PM watch.

- Added the in-window RAG surface to agent-team-loop (mw-rag-window-parity): `/mw rag <list|probe|audit|sync|init> [args]` is a thin wrapper over `mw.py rag` (Python stays the single source of parsing and validation; the wrapper appends `--project=<control root>`) that maps the CLI exit codes onto notify levels - 0 info, 1 warning, 2 error - so `rag audit`'s "found findings" exit 1 is not reported as a failure, truncates output longer than 30 lines while appending the full command as a hint, and renders the usage line (listing all five subcommands) without spawning anything for an unknown or empty subcommand. `/mw doctor` now prints the RAG status row byte-identically to `mw.py doctor`'s text line - same `rag.exists or rag.enabled` gate, same field order, probe names sorted, 12-character fingerprint, `required_missing` only when non-zero - so a project without any RAG configuration shows no row in either place. The window's `RAG_SUBCOMMANDS` whitelist is locked to `mw.py`'s `_RAG_ACTIONS` by a test, and the `findMwPy()`-missing branch is covered by a test that mocks `node:fs`.

### Fixed

- Fixed `runAgenticScript` (agent-team-loop) spawning framework gate scripts without forcing UTF-8 stdio: a Windows console default (cp936) read back as UTF-8 turned the framework's Chinese gate diagnostics into `?`. The child interpreter now gets `-X utf8`, matching the mw Python side (`autopilot/advance.py`).
...
- Fixed agent-team-loop worker exits leaking the bash tool's in-flight child process trees as orphans on Windows: every `process.exit(1)` path (idle/wall watchdog timeout, the `agent_settled` catch, the startup task refusal, a missing task file) and the process `'exit'` safety-net hook now kill the tracked child trees first (`taskkill /F /T` / `kill(-pgid)`), always after the synchronous trace/output writes so the PM readback still lands. Motivated by the 2026-09-19 incident: a watchdog-killed worker's hung bash-tool child survived its parent and spun to a 151 GB working set.
- Fixed the worker dispatch docs gate blocking every fresh AgenticTask key (mw-worker-visibility-gate): the gate required `design.md` and a `design-*` note before any worker could run, so a key sitting in SPEC could never dispatch the very research worker that produces the design evidence, and the documented workaround was to dispatch key-owned work under `_scratch` - splitting the evidence trail (task.md and the queue row under the throwaway key, the notes under the real one). The design-side requirements are now applied from the DESIGN phase onward, the phase is read once through the shared gate, and a key with an incomplete spec side is still refused with the same gap list and hint.
...
- Fixed `agent-team-loop` worker-failure summaries reporting a generic crash/timeout guess when the launcher recorded an exact spawn-failure reason (e.g. a missing route credential) in `worker.log`; the poll loop now surfaces that line instead.
- Changed `agent-team-loop` worker summaries and progress to be scoped per window to an explicitly watched key (`/mw-watch <key> | off`, or `/pm-key switch|new`) instead of matching every key marked active in the shared `_index.parallel`, which made all windows notify about every active key. The watched key's live progress (phase, per-worker status, failure reasons) now renders in a persistent widget below the editor, refreshed every poll tick; windows without a watched key stay silent.
- Added the `/mw-watch` command to toggle the per-window bottom progress widget for one AgenticTask key (or `_scratch`).
- Changed `/pm-key switch` into an explicit takeover: it stamps the key's `_index.parallel` claimId with the window identity (`host:pid`), refuses to stomp a claim held by another live window unless `--force` is passed, and silently recovers claims whose holder process has exited (legacy timestamp claims count as stale). `/pm-key new` claims immediately and rejects duplicate keys; `/pm-key list` shows the claim owner.
- Added session persistence for the watched key: `/pm-key switch|new` and `/mw-watch` record it in the session file, and on restart/resume the window restores its watch, re-renders the bottom widget, and quietly re-claims a stale claim (deferring with a warning when another live window now holds the key).
- Added spec-write auto-takeover: when a window's agent writes or edits `.agenticdoc/{key}/spec.md`, `design.md`, or `plan.md` (write/edit tools), the window automatically claims the key (registering it in `_index.parallel` if unknown, marking other active keys idle), watches it, and renders the bottom widget �?no explicit `/pm-key` command needed. A live foreign claim downgrades the trigger to watch-only with a warning; repeated doc writes are idempotent.
- Added the `switch_key` agent tool: same takeover semantics as `/pm-key switch`, callable mid-conversation when the user asks to take over or switch to a key, or before executing a key's tasks. Its prompt guideline instructs the agent to ask the user first when they haven't mentioned the key.
- Added the phase-docs dispatch gate: worker tasks under a real AgenticTask key are only dispatched when the key is fully documented �?`spec.md` and `design.md` (>= 500 bytes each) plus at least one `evidence/research/spec-*.md` and one `evidence/research/design-*.md` note (a zero-research declaration counts), mirroring `advance_phase.py`'s design/plan gates. Enforced in the `dispatch_worker` tool, the `/worker` command, and the `agent_settled` task scan (undocumented keys are skipped and reported once per session); `_scratch` stays the ad-hoc escape hatch. The bottom widget header now shows the docs state (`S+ D- ev:1/2`), and writing `design.md`/`plan.md` immediately warns when the previous phase's research evidence is missing.
- Added the evidence re-review nudge: the moment a phase doc and its research evidence coexist (spec.md/design.md written with notes present, or a research note written for an already-written doc), the window agent receives a re-review instruction �?claims-vs-evidence audit with unverified assertions marked, numbers re-anchored to measured data with caliber notes, AC-vs-enumeration consistency, reuse-cost assumptions, and falsifiability/machine-observable oracles �?and applies corrections/annotations directly to the doc. Fires once per (key, phase) per session; writing `spec.md` without research notes now warns symmetrically.
- Added the pm-state.md interface guard: write/edit tool calls that create, change, or remove the machine-owned - Phase: / - Claim-Id: lines in .agenticdoc/{key}/pm-state.md are hard-blocked with a pointer to dvance_phase.py; byte-identical rewrites that leave those lines untouched stay allowed. This closes the hand-edit bypass path that skipped phase gates.
- Added the takeover audit: /pm-key new|switch, the switch_key tool, and spec-write auto-takeover replay the phase-gate chain up to the key's current phase (spec/design/plan gates + tasks/ presence) plus a pm-state vs _index.parallel divergence check, surfacing the warnings as notifications — a TS mirror of udit_phase.py.
- Changed the phase-docs dispatch gate to also require a numbered acceptance criterion (AC-NNN) in spec.md, and a non-empty §0 Goal Alignment section naming the expected benefit whenever goal.md is established, matching dvance_phase.py's design gate.
- Added an ack flow for terminal `agent-team-loop` worker rows: `/mw ack <task-key>|all` and the `ack_worker_result` tool persist acknowledgements in `.agenticdoc/_workers.acked` (workers-lock-guarded, idempotent, restart-surviving), `list_tasks` badges acked rows, and the terminal-result continuation hint directs the PM to ack after absorbing a result. Running/pending rows are rejected with a reason and never written.
- Changed the watch widget's task rows into three sections: live (running, then pending) and unacked failed/needs-clarification rows are always shown in full, while done and acked terminal rows fold into the five newest with a `+N more` line (was: every row capped at six); the header carries an `N unhandled` count so unacked failures stay visible until processed. `.agenticdoc/_workers.parallel` row format is untouched.
- Fixed terminal watch rows showing the frozen first line of `## Summary` regardless of outcome: failed rows show the spawn-failure reason or the `## Exit Reason` first line, needs-clarification rows show `## Questions` (falling back to the worker.log tail with a 256KB read guard), and done rows show the new `## TL;DR` first section written by `writeOutput` (normalized single-line conclusion, <=100 chars, leading markdown markers stripped), falling back to a marker-stripped Summary line for old reports; the worker starter prompt and deadline steer now direct the final reply to open with a one-line conclusion.
- Fixed orphaned `running` rows never converging after `mw serve` restarts or kills: the launcher reconciles rows it does not own every poll — `[END]` trace evidence maps the exit code to its terminal status (0=done, 2=needs-clarification, else failed), a lone output.md fails as unverifiable, and evidence-less rows fail as presumed dead after a 90-minute silence window (env `PI_WORKER_ORPHAN_DEAD_MIN`), with each launcher publishing a fresh beat (`.mw/launcher-beat.<pid>`) that makes the silence rule yield to another live launcher.
- Fixed the `agent-team-loop` goal nudge breaking headless sessions: `pi -p` in any project without an established `goal.md` failed with "Agent is already processing" because the session-start nudge's `sendUserMessage` started a generation that collided with the queued print-mode prompt. The nudge now skips headless (no-UI) sessions entirely, and the evidence re-review nudge queues with `deliverAs: "followUp"` instead of throwing mid-run when it fires from tool-write events.
- Added stale-serve detection and `/mw restart`: mw serve stamps `.mw/serve.meta` (pid + start time), the extension compares it against the mw source tree's newest mtime at session start and in `/mw status`, and `/autopilot enable` restarts a stale serve before promising a conductor — a serve that predates the current code silently misses features (e.g. conductor supervision) and the old enable flow promised a conductor that never spawned. `/mw restart` runs a graceful stop-request → start cycle; in-flight workers are adopted by the new launcher's orphan reconcile.
- Fixed `agent-team-loop` worker dispatches without an explicit key landing under the globally-latest active `_index.parallel` row — another window's key — whenever multiple windows each claimed a different key (every takeover demotes the other active rows, so the shared pointer always names the most recent claimer). Owner resolution now follows this window's own claim first (the watched key whose claimId matches the window's `host:pid`, re-bound on restart); a stale or unclaimed watch falls back to the global active row only when no other live window holds it, degrading to `_scratch` with a once-per-session warning otherwise. An explicit `key` still wins.
- Fixed `agent-team-loop` going permanently dead after any session replacement (`/new`, `/resume`, `/fork`, `/reload`): the double-load guard's process-global activation flag was never cleared, so the re-activation pi runs when rebuilding the runtime no-op'd and the replaced session silently lost every command, tool, handler, and the watch widget — `/pm-key switch` then fell through to the LLM as an unknown command instead of taking over the key and rendering the bottom widget. The guard now re-arms on `session_shutdown`, preserving the same-load double-import protection while letting session replacements and reloads fully re-activate.
- Fixed the evidence re-review nudge hijacking the window agent's turn: when a phase doc and its research notes coexisted, the framework injected a "please immediately run the review" message via `sendUserMessage`, which the model executed as a direct user order — rewriting the spec/design doc with no consent and no chance to veto, from any window that touched either file, regardless of its watch key. The "re-review when evidence is generated" detection is kept, but downgraded to a UI `notify` info notice (once per key and phase per session) that never enters the model context: the framework flags the moment; the user decides when (and whether) to run the review.
...
- Fixed the RAG runtime-evidence gap for VC-018 (mw-rag-integration-fix): `rag-required.test.ts` now drives `workerModeActivate` for a `rag-research` task and asserts the validator reaches the compliant `<server>-<slug>.md` at the real `.agenticdoc/<key>/rag/` layout (`[VERIFY] VC-106: runtime_doc_found=true sections=6 verdict=ok`), with a negative case proving the same runtime path emits `rag-required-missing` when the doc is absent.

- Fixed the RAG config parity gaps between the extension loader and the Python loader (mw-rag-window-parity): a missing or `null` `skill.dir` is now accepted (meaning the control workspace root, resolved at the call site) instead of failing with `invalid-shape` where `mw rag list` accepted it, and the fingerprint of such a config stays byte-identical to `mw_common.rag_fingerprint` (the parsed value stays `null` and is never normalized to `"."`, which would have split the two hashes). `mcp.url`, `mcp.token_env`, `skill.dir`, and `skill.cli_entry` are now stripped like Python's `_rag_finalize_mcp`/`_rag_finalize_skill`, so a whitespace-padded value can no longer yield two different fingerprints for one config file. Known residual divergence (declared, not fixed here): `path_roots_file` is not stripped, because Python strips the field but computes the digest from the untrimmed string.

- Fixed the extension-side config read-modify-write losing updates (mw-autopilot-verify-cli): two writers interleaved their read and write windows and the later write erased the earlier one - measured at 28 lost updates out of 30 rounds for two console writers, and reproducible with two TypeScript writers alone within 30 rounds. The four console config handlers now serialise that window under the dedicated `<root>/.mw/autopilot-config.lock`.
