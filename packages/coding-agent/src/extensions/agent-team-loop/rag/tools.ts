...
import {
	...
	type RagEnvelope,
	type RagErrorKind,
	type RagToolError as RagToolErrorShape,
	ragToolCalls,
} from "./adapter.ts";
import type { BudgetState } from "./budget.ts";
import { RAG_HEARTBEAT_INTERVAL_MS, withHeartbeat } from "./budget.ts";
import { callCli } from "./cli-bridge.ts";
import {
	loadRagConfig,
	type RagConfig,
	RagConfigError,
...
import { RAG_PROMPT_GUIDELINES } from "./guidelines.ts";
import { McpSession, RagToolError } from "./mcp-client.ts";
 ...
 */
export function ragCallTimeoutMs(logicalTool: string, config: RagCallTimeoutConfig): number {
...
export interface RagBudgetOps {
	...
	reserveChat(): { ok: true } | { ok: false; kind: "budget"; message: string };
	settleChat(ok: boolean, kind: RagErrorKind | null): void;
	accumulate(ms: number): void;
	checkCumulative(): { ok: true } | { ok: false; kind: "budget"; message: string };
	checkWall(nowMs: number): { ok: true } | { ok: false; kind: "budget"; message: string };
...
/** Parameter bag every tool passes through the shared chain. */
...
async function probeAndRegister(pi: ExtensionAPI, runtime: RagRuntime): Promise<void> {
	await Promise.all(
...
function withReachability(runtime: RagRuntime, description: string): string {
...
function registerBaseTools(pi: ExtensionAPI, runtime: RagRuntime): void {
	...
	pi.registerTool({
		...
		execute: async (_toolCallId, params, signal, onUpdate) =>
			await callRag(runtime, "rag_search", params, signal, wrapOnUpdate(onUpdate)),
	});
	...
	pi.registerTool({
		...
		execute: async (_toolCallId, params, signal, onUpdate) =>
			await callRag(runtime, "rag_symbol", params, signal, wrapOnUpdate(onUpdate)),
	});
	...
	pi.registerTool({
		...
		execute: async (_toolCallId, params, signal, onUpdate) =>
			await callRag(runtime, "rag_graph", params, signal, wrapOnUpdate(onUpdate)),
	});
	...
	pi.registerTool({
		...
		execute: async (_toolCallId, params, signal, onUpdate) =>
			await callRag(runtime, "rag_impact", params, signal, wrapOnUpdate(onUpdate)),
	});
	...
	pi.registerTool({
		...
		execute: async (_toolCallId, params, signal, onUpdate) =>
			await callRag(runtime, "rag_sources", params, signal, wrapOnUpdate(onUpdate)),
	});
	...
	pi.registerTool({
		...
		execute: async (_toolCallId, params, signal, onUpdate) =>
			await callRag(runtime, "rag_feedback", params, signal, wrapOnUpdate(onUpdate)),
	});
}

/** Register the research-only tool exactly once (idempotent). */
function ensureRagChatTool(pi: ExtensionAPI, runtime: RagRuntime): void {
	...
	pi.registerTool({
		...
		execute: async (_toolCallId, params, signal, onUpdate) =>
			await callRag(runtime, RAG_CHAT_TOOL_NAME, params, signal, wrapOnUpdate(onUpdate)),
	});
}

/** Tool names the given task type may activate out of the RAG surface. */
export function ragToolNamesForType(rt: RagRuntime, type: string): string[] {
	const names: string[] = [...RAG_BASE_TOOL_NAMES];
 ...
 * server list.
 */
export function validateRagEnabled(projectDir: string): { ok: true } | { ok: false; message: string } {
	try {
...
function wrapOnUpdate(
	...
	return (message: string) => onUpdate({ content: [{ type: "text", text: message }], details: undefined });
}

/** Which transport actually produced one leg's response (T-05 evidence). */
interface LegOutcome {
	response: unknown;
	via: "mcp" | "cli";
	/** True when this leg reached the cli because the mcp `connect` failed (D-005). */
	fallback: boolean;
}
...
async function callRag(
	...
	for (const [key, value] of Object.entries(params)) {
		if (key !== "server" && value !== undefined) args[key] = value;
	}
	...
	if (explicitSource === undefined && source !== null) args.source = source;
	...
	if (tool === "rag_search" && args.multi_rounds === undefined && args.auto_rewrite === undefined) {
		const explicitRewrite =
			runtime.config.roles[runtime.role]?.rewrite ?? runtime.config.phases[runtime.phase]?.rewrite;
...
function ragTokenEnvNames(config: RagConfig): string[] {
	const names = new Set<string>();
	for (const entry of Object.values(config.servers)) {
		const name = entry.mcp?.tokenEnv;
		if (name !== undefined && name !== null && name.length > 0) names.add(name);
	}
	return [...names];
}
 ...
 */
function evidenceResults(envelope: RagEnvelope): number {
...
function redactToolError(error: RagToolErrorShape, tokenEnvNames: string[]): RagToolErrorShape {
...
function redactDetail(detail: unknown, tokenEnvNames: string[]): unknown {
...
function resolvePathRootsFile(controlRoot: string, entry: RagServerEntry): string | null {
 ...
 * spawning the CLI.
 */
export function resolveCliDir(controlRoot: string, dir: string | null): string {
 ...
 */
function retag(error: unknown, server: string, tool: string): RagToolErrorShape {
