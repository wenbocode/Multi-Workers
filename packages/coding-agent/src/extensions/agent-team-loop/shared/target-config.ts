...
import { isMap, isScalar, parse, parseDocument, type YAMLMap } from "yaml";
...
function normalizeRoot(raw: string, controlRoot: string): string {
	...
	try {
		...
		for (;;) {
			...
			if (parent === dir) return resolved; // reached the drive root with nothing existing
			try {
...
const V1_SHAPE_KEYS = ["mode", "game", "engine", "uproject"] as const;
...
function detectShape(raw: Record<string, unknown>, fileExists: boolean): "none" | "v1" | "v2" {
...
export function decideActiveMode(input: DecideActiveModeInput): ActiveModeResult {
	const ep: string[] = [];
	...
	if (input.fileShape === "v2") {
		...
		if (active === null || (active !== "single" && active !== "dual" && active !== "partition")) {
			...
			fail(
				"invalid-config",
				`target.yml: 'active' must be one of 'single', 'dual', 'partition', got ${
		...
		if (active === "single") return { mode: "single", block: null }; // row 5
	...
	if (ep.length === 2) return { mode: "partition", block: null }; // row 9
	if (ep.length === 1) {
		// row 10
...
function checkV2TopWhitelist(raw: Record<string, unknown>): void {
...
function v2Block(raw: Record<string, unknown>, name: "dual" | "partition"): Record<string, unknown> {
...
function checkBlockKeys(block: Record<string, unknown>, name: "dual" | "partition", allowed: Set<string>): void {
...
function checkRootsKeyTypes(rootsAst: YAMLMap): void {
...
function readRootsAst(controlRoot: string): YAMLMap | null {
	const file = targetYmlPath(controlRoot);
	let text: string;
	try {
		text = fs.readFileSync(file, "utf-8");
...
function parseRoots(value: unknown, controlRoot: string, rootsAst: YAMLMap | null): Record<string, string> {
	if (value === undefined || value === null) return {};
	if (typeof value !== "object" || Array.isArray(value)) {
	...
	for (const [name, rawPath] of Object.entries(value as Record<string, unknown>)) {
...
function relationNorm(p: string): string {
...
function checkRootRelation(parentRoot: string, partitionRoot: string): void {
...
function loadPartitionConfig(
	...
	envParent: string | null,
	envPartition: string | null,
	rootsAst: YAMLMap | null,
): WorkspaceConfig {
	let parentRaw: unknown = block === null ? null : block.parent;
	let partitionRaw: unknown = block === null ? null : block.partition;
...
function renderPartitionCommand(command: string, config: WorkspaceConfig): string {
	let out = command;
