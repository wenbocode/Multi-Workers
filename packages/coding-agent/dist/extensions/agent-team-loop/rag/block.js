...
import { DEFAULT_RAG_CHAT_BUDGET, DEFAULT_RAG_TIME_BUDGET_S, ragFingerprint, resolveDefaults, } from "./config.js";
 ...
 */
export function renderRagBlock(config, meta) {
    ...
    if (requiredRoles.length > 0)
        lines.push(`[mw] Required roles: ${requiredRoles.join(", ")}`);
    const requiredPhases = Object.entries(config.phases)
        .filter(([, spec]) => spec.require === true)
        .map(([name]) => name)
        .sort();
    if (requiredPhases.length > 0)
        lines.push(`[mw] Required phases: ${requiredPhases.join(", ")}`);
    lines.push(`[mw] Default server: ${server ?? "none"}`);
    lines.push(`[mw] Default source: ${source ?? "none"}`);
    lines.push(`[mw] Rewrite: ${rewrite ? "true" : "false"}`);
    lines.push(`[mw] Chat budget: ${resolveBudget(meta.chatBudget, roleSpec?.chatBudget, config.budgets.chat, DEFAULT_RAG_CHAT_BUDGET)}`);
    lines.push(`[mw] Time budget: ${resolveBudget(meta.timeBudgetS, roleSpec?.timeBudgetS, config.budgets.timeS, DEFAULT_RAG_TIME_BUDGET_S)}s`);
