...
import { RagToolError } from "./mcp-client.js";
const SECRET_ENV_KEY_RE = /(token|secret|password|passwd|api[_-]?key|credential)/i;
export function resolveRagPython(env = process.env) {
    const override = env.MW_RAG_PYTHON?.trim();
    if (override !== undefined && override.length > 0)
        return override;
    return process.platform === "win32" ? "python" : "python3";
}
export async function callCli(cliEntry, name, args, opts) {
    ...
    if (opts.signal.aborted)
        throw cliError("timeout", server, name, "call cancelled before spawn");
    return await new Promise((resolve, reject) => {
        ...
        child.on("error", (error) => {
            ...
            settled = true;
            cleanup();
            reject(cliError("connect", server, name, `failed to start ${interpreter}: ${error.message}`, {
                code: error.code,
            }));
        });
        child.on("close", (code) => {
