...
import { parseCitation } from "./adapter.js";
import { ragRequiredMissingLine } from "./evidence.js";
/** The six fixed section titles, verbatim from the SKILL.md standard. */
export const RESEARCH_DOC_SECTIONS = ["查询", "结论", "引用", "未解决", "快照", "影响面"];
/** The rendered headings (`## <title>`), in the required order. */
export const RESEARCH_DOC_HEADINGS = RESEARCH_DOC_SECTIONS.map((section) => `## ${section}`);
/** `rag/` directory under a key dir (`.agenticdoc/<key>/rag`). */
export function researchDocDir(keyDir) {
...
export function splitResearchDocSections(text) {
...
export function scanCitations(text) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        for (const candidate of citationCandidates(trimmed)) {
            if (parseCitation(candidate) !== null)
...
function collectCitations(lines) {
    ...
    for (const line of lines) {
        ...
        if (candidates.length === 0) {
            if (isCitationEntryLine(trimmed))
                unparseable.push(trimmed);
            continue;
        }
        for (const candidate of candidates) {
            if (parseCitation(candidate) === null)
                unparseable.push(candidate);
            else
 ...
 */
export function validateResearchDoc(keyDir) {
    const findings = [];
...
//# sourceMappingURL=research-doc.js.map