...
import { rawTargetYml } from "../shared/target-config.js";
...
function asObject(value, field) {
    if (!isObject(value))
        fail("invalid-shape", `rag config: field '${field}' must be a mapping`);
    return value;
}
...
function optionalBoolean(value, field, fallback) {
    ...
    if (typeof value !== "boolean")
        fail("invalid-shape", `rag config: field '${field}' must be a boolean`);
    return value;
}
...
function stringArray(value, field) {
    ...
    if (!Array.isArray(value))
        fail("invalid-shape", `rag config: field '${field}' must be a list of strings`);
    return value.map((entry) => requireString(entry, field));
}
function checkKeys(value, allowed, where) {
    const extra = Object.keys(value).filter((key) => !allowed.has(key));
    if (extra.length > 0) {
        fail("unknown-key", `rag config: unexpected key(s) in ${where}: ${extra.join(", ")} (allowed: ${[...allowed].join(", ")})`);
    }
}
 ...
 */
export function machineRagServersPath(env = process.env) {
...
const SERVER_FIELDS = ["transport", "adapter", "path_roots_file", "sources", "capabilities", "mcp", "skill"];
const SERVER_KEYS = new Set(SERVER_FIELDS);
...
function parseTransport(value, field) {
    if (value === "mcp" || value === "skill" || value === "both")
        return value;
...
function parseMcp(raw, name) {
    ...
    if (url === null)
        fail("mcp-missing-url", `rag server '${name}': mcp.url is required`);
    return {
        url,
...
function parseSkill(raw, name) {
    ...
    if (cliEntry === null)
        fail("skill-missing-cli", `rag server '${name}': skill.cli_entry is required`);
    return {
        dir,
        cliEntry,
...
function parseCapabilities(raw, name) {
    if (raw === null)
        return { graph: false, chat: false, rewrite: false };
    return {
 ...
 */
export function mergeServerEntry(machine, project, name) {
    const machineObj = machine === undefined || machine === null ? null : asObject(machine, `server '${name}' (machine)`);
    const projectObj = project === undefined || project === null ? null : asObject(project, `server '${name}' (project)`);
    if (machineObj !== null)
        checkKeys(machineObj, SERVER_KEYS, `server '${name}' (machine)`);
    if (projectObj !== null)
        checkKeys(projectObj, SERVER_KEYS, `server '${name}' (project)`);
    ...
    for (const field of SERVER_FIELDS) {
        ...
        if (inMachine && machineValue === null && !inProject)
            continue;
        ...
        if (nestedFields !== undefined) {
            ...
            if (inProject && projectValue !== undefined && projectValue !== null) {
                ...
                for (const key of Object.keys(projectBlock)) {
                    if (projectBlock[key] === null)
                        delete nested[key];
                    else
                        nested[key] = projectBlock[key];
    ...
    const transport = rawTransport === undefined || rawTransport === null
        ? "mcp"
    ...
    if ((transport === "skill" || transport === "both") && skill === null) {
        fail("skill-missing-cli", `rag server '${name}': transport '${transport}' requires a 'skill' block with a cli_entry`);
    }
    return {
        transport,
        mcp,
...
function parseEnabled(value) {
    if (value === undefined || value === null)
        return [];
    if (!Array.isArray(value))
        fail("invalid-shape", "target.yml rag.enabled must be a list of server names");
    return value.map((entry) => {
        if (typeof entry !== "string")
            fail("invalid-shape", "target.yml rag.enabled must be a list of server names");
        if (entry.trim() === "")
            fail("enabled-empty-entry", "target.yml rag.enabled must not contain empty entries");
        return entry;
    });
}
...
function unknownServer(name, visible) {
    fail("unknown-server", `unknown rag server '${name}' (visible: ${visible.length > 0 ? visible.join(", ") : "none"})`);
}
/**
 ...
 */
export function loadRagConfig(controlRoot) {
    ...
    for (const name of enabled) {
        if (!(name in servers))
            unknownServer(name, visible);
    }
    const defaultServer = optionalString(rag.default_server, "target.yml rag.default_server");
    if (defaultServer !== null && !(defaultServer in servers))
        unknownServer(defaultServer, visible);
    const roles = parseRoleSpecs(rag.roles);
    for (const spec of Object.values(roles)) {
        if (spec.server !== undefined && !(spec.server in servers))
            unknownServer(spec.server, visible);
    }
 ...
 */
export function rewriteDefaults(role, phase, capabilityRewrite, explicit) {
 ...
 * role/phase (AC-006).
 */
export function resolveDefaults(config, role, phase) {
...
export function requiredFor(config, role, phase) {
...
function roleSpecToSnake(spec) {
    ...
    if (spec.server !== undefined)
        out.server = spec.server;
    if (spec.source !== undefined)
        out.source = spec.source;
    if (spec.require !== undefined)
        out.require = spec.require;
    if (spec.rewrite !== undefined)
        out.rewrite = spec.rewrite;
    if (spec.chatBudget !== undefined)
        out.chat_budget = spec.chatBudget;
    if (spec.timeBudgetS !== undefined)
        out.time_budget_s = spec.timeBudgetS;
    return out;
}
...
export function ragFingerprint(config, enabledOnly) {
    ...
    for (const role of Object.keys(config.roles))
        roles[role] = roleSpecToSnake(config.roles[role]);
    const phases = {};
    for (const phase of Object.keys(config.phases))
        phases[phase] = phaseSpecToSnake(config.phases[phase]);
    const payload = {
        enabled: names,
        servers,
