...
export function autopilotDir(projectDir) {
    return path.join(projectDir, ".agenticdoc", "_autopilot");
}
export function roadmapPath(projectDir) {
...
export function timelinePath(projectDir) {
...
export function gatesLockPath(projectDir) {
...
export function configLockPath(projectDir) {
...
export const BOOL_FIELDS = ["enabled", "paused", "xkey_repair"];
...
function configReviver(key, value, context) {
    if (typeof value !== "number" || !Number.isInteger(value))
        return value;
    if (!Object.hasOwn(INT_RANGES, key))
        return value;
    const source = context?.source;
    if (source !== undefined && !INT_LITERAL_RE.test(source))
        throw new IntegerLiteralError(key, source);
    return value;
}
...
export function validateConfigData(data) {
    ...
    for (const [field, [lo, hi]] of Object.entries(INT_RANGES)) {
        ...
        if (value < lo)
            errors.push(`${field}: must be >= ${lo}, got ${value}`);
        if (hi !== null && value > hi)
            errors.push(`${field}: must be <= ${hi}, got ${value}`);
    }
    return errors;
}
...
export function readConfig(projectDir) {
    ...
    try {
        ...
        if (err instanceof IntegerLiteralError)
            return { ok: false, error: err.message };
        return { ok: false, error: `cannot parse ${file}: ${String(err)}` };
    }
    const errors = validateConfigData(data);
    if (errors.length > 0)
        return { ok: false, error: `invalid _autopilot/config.json: ${errors.join("; ")}` };
...
export function parseRoadmapText(text) {
    ...
    for (let lineno = 1; lineno <= lines.length; lineno++) {
        ...
        if (line.startsWith("|")) {
            if (cur !== null && inKeysTable) {
                ...
                if (cells.length !== 3) {
                    warnings.push(`line ${lineno} (stage ${cur.stage.number}): keys table row must have 3 columns (key | role | depends_on), got ${cells.length}`);
                    continue;
                }
                const key = cells[0] ?? "";
                if (key === "") {
            ...
            if (cur !== null) {
                if (name === "goal" || name === "status" || name === "key-status") {
                    if (cur.seenFields.has(name)) {
                        warnings.push(`line ${lineno} (stage ${cur.stage.number}): duplicate '> ${name}:' line (last one wins)`);
                    }
                    else {
                        cur.seenFields.add(name);
                    }
                    if (name === "goal")
                        cur.stage.goal = value;
                    else if (name === "status") {
    ...
    finish();
    if (stages.length === 0)
        warnings.push("no '## Stage <number>: <title>' sections found");
...
function parseKeyStatusLine(value, stage, lineno, warnings) {
    for (const part of value.split(",")) {
        ...
        if (eq <= 0) {
            warnings.push(`line ${lineno} (stage ${stage.number}): malformed key-status entry '${entry}' (expected <key>=<status>)`);
            continue;
        }
        ...
        if (key in stage.keyStatus) {
            warnings.push(`line ${lineno} (stage ${stage.number}): duplicate key-status entry for '${key}' (last one wins)`);
        }
        stage.keyStatus[key] = status;
    }
}
export function readRoadmap(projectDir) {
...
export function parseGateFile(text, file) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (field === null)
            throw new GateFormatError(`${file}: invalid frontmatter line ${i + 1}: '${line}'`);
        const name = field[1] ?? "";
    ...
    if (!closed)
        throw new GateFormatError(`${file}: frontmatter never closes with a '---' line`);
    const required = (name) => {
...
export function listGates(projectDir) {
    ...
    for (const entry of entries) {
        ...
        if (m === null || !entry.isFile())
            continue; // non-gate files and leftover .tmp files
...
function rotationChain(timelinePath) {
...
export function queryTimeline(timelinePath, watermark, evFilter) {
...
export function watermarkFromSince(timelinePath, sinceIso) {
    ...
    for (const source of [...rotationChain(timelinePath), timelinePath]) {
        for (const ev of readTimelineEvents(source).events) {
            ...
            if (!Number.isNaN(ts) && ts <= since && ev.seq > mark)
                mark = ev.seq;
        }
    }
    return mark;
...
export function parseTaskLabels(taskMdPath) {
    let text;
    try {
...
export function usedRounds(workersDirs) {
    const units = new Map();
    ...
    for (const [loop, attempts] of units)
        result.set(loop, attempts.size);
    return result;
}
...
function keyRounds(perLoop, key, budget) {
    ...
    for (const [loop, used] of perLoop) {
        if (loop.startsWith(`l2:${key}:`))
            l2 = Math.max(l2, used);
        else if (loop === `l3:${key}`)
            l3 = Math.max(l3, used);
        else if (loop.startsWith(`exec:${key}:`) || loop === `repair:${key}`)
            retry = Math.max(retry, used);
    }
    return {
 ...
 * views plus warnings. */
export function deriveStatusModel(projectDir) {
...
export function renderStatusText(model) {
    ...
    if (status.stage.current > 0) {
        ...
        for (const k of status.stage.keys) {
            lines.push(`  ${k.key} | phase=${k.phase} | state=${k.state} | L2 ${k.rounds.l2.used}/${k.rounds.l2.max}` +
                ` | L3 ${k.rounds.l3.used}/${k.rounds.l3.max} | retry ${k.rounds.retry.used}/${k.rounds.retry.max}`);
        }
        if (status.stage.keys.length === 0)
            lines.push("  (no keys)");
    }
    else {
    ...
    for (const g of status.gates)
        lines.push(`  ${g.id} ${g.kind} ${g.status}`);
    lines.push(`timeline: seq=${status.timeline.seq} ts=${status.timeline.ts}`);
    lines.push(`config: enabled=${status.config.enabled} | poll_interval_sec=${status.config.poll_interval_sec}` +
