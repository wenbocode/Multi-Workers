...
import { readPhaseDocs } from "./phase-docs.js";
/**
...
function pmStateTarget(toolName, args, projectDir, agenticdocRoot) {
    ...
    if (parts.length !== 2 || parts[1] !== "pm-state.md")
        return undefined;
    const key = parts[0] ?? "";
 ...
 * lines untouched).
 */
export function pmStateInterfaceViolation(toolName, args, projectDir, agenticdocRoot, readFile = defaultReadFile) {
    ...
    if (toolName === "write") {
        ...
        if (next.length === 0)
            return undefined; // rewrite without interface lines: allowed, advance guards handle drift
        const cur = interfaceLines(readFile(target.absPath) ?? "");
        if (cur.length === next.length && cur.every((l, i) => l === next[i]))
            return undefined;
    ...
    for (const e of edits) {
        ...
        if ((typeof oldText === "string" && INTERFACE_LINE.test(oldText)) ||
 ...
 */
export function registerPmStateGuard(pi, projectDir, agenticdocRoot) {
...
export function readPmStatePhase(agenticdocRoot, key) {
    try {
 ...
 */
export function phaseAuditWarnings(agenticdocRoot, key, indexPhase) {
    if (key === SCRATCH_WORKERS_KEY)
        return [];
    const warnings = [];
    const pmPhase = readPmStatePhase(agenticdocRoot, key);
    if (!pmPhase || pmPhase === "(pending)" || pmPhase.toUpperCase() === "INIT")
        return warnings;
    ...
    if (indexPhase && indexPhase !== "—" && indexPhase.toUpperCase() !== upper) {
        warnings.push(`[audit] INDEX-DIVERGENCE: pm-state.md phase=${upper} != _index.parallel phase=${indexPhase} — phase changes must go through advance_phase.py (it syncs both sources); divergence means a hand-edit bypassed the gates.`);
    }
    const docs = readPhaseDocs(agenticdocRoot, key);
    if (rank >= PHASE_ORDER.indexOf("DESIGN")) {
        if (!docs.spec)
            warnings.push("[audit] design gate: spec.md missing or under 500 bytes.");
        if (docs.specEvidence < 1)
            warnings.push("[audit] design gate: evidence/research/spec-*.md missing (>= 1 note; zero-research declaration counts).");
        if (!docs.specS0)
            warnings.push("[audit] design gate: spec.md missing a non-empty §0 Goal Alignment section with 预期收益 (goal.md is established).");
        if (!docs.specAC)
            warnings.push("[audit] design gate: spec.md has no numbered acceptance criteria (AC-NNN).");
    }
    if (rank >= PHASE_ORDER.indexOf("PLAN")) {
        if (!docs.design)
            warnings.push("[audit] plan gate: design.md missing or under 500 bytes.");
        if (docs.designEvidence < 1)
            warnings.push("[audit] plan gate: evidence/research/design-*.md missing (>= 1 note; zero-research declaration counts).");
    }
    if (rank >= PHASE_ORDER.indexOf("EXECUTE")) {
        try {
            ...
            if (n < 1)
                warnings.push("[audit] execute gate: tasks/ has no .md files.");
        }
        catch {
            warnings.push("[audit] execute gate: tasks/ directory missing (mw-dispatch flows: keep a pointer task.md under tasks/ so the key stays auditable).");
        }
    }
    return warnings;
}
//# sourceMappingURL=pm-state-guard.js.map