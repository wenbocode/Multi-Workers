...
export const PROVIDER_ID_TO_PREFIX = {
    timi: "timi",
    anthropic: "claude",
    "openai-codex": "codex",
    deepseek: "deepseek",
...
export function parseModelValue(value) {
    ...
    if (idx < 0)
        return { prefix: "", modelId: value.trim() };
...
export const DISPATCHABLE_TYPES = ["coding", "review", "research", "rag-research"];
...
export function recordWindowModel(cwd, model) {
    if (!model || !isFrameworkProject(cwd))
        return;
...
export function settingsDefaultModel() {
...
export function hasCliModelFlag() {
...
export async function applyMainModelConfig(pi, ctx) {
    if (hasCliModelFlag() || settingsDefaultModel())
        return;
    ...
    if (!model) {
        ctx.ui.notify(`dispatch.yml main=${value} not found in the model registry — leaving the window model unchanged`, "error");
        return;
    }
...
export function registerMainWindowModel(pi) {
...
//# sourceMappingURL=dispatch-models.js.map