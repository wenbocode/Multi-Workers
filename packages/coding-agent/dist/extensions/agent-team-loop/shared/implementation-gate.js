...
const CODE_EXTENSIONS = [".ts", ".tsx", ".js", ".cjs", ".mjs", ".py"];
const CODE_EXCLUDED_SEGMENTS = ["node_modules", "dist", ".tmp"];
/** bash write verbs whose FILE OPERAND side is a write target. */
const WRITE_VERBS = new Set(["tee", "cp", "mv", "rm", "sed"]);
const GATE_EXPLANATION = "Create a key first: run /agentic (the AgenticTask flow), or write .agenticdoc/<key>/spec.md — update_index.py " +
...
export function resolveGateRoot(env = process.env) {
...
export function currentClaimId() {
 ...
 * exclusions. */
export function isCodePath(rawPath, root = resolveGateRoot(), cwd = process.cwd()) {
...
function parseIndexRows(content) {
    ...
    if (keyIdx < 0 || statusIdx < 0 || claimIdx < 0)
        return undefined;
...
export function hasActiveKeyClaim(root, claimId, readIndexFile = defaultReadTextFile) {
...
export function findFreshMiniSpec(root, now = Date.now(), maxAgeMs = MINI_SPEC_FRESH_MS) {
    let entries;
    try {
...
function parseBashSegments(command) {
    ...
    const endSegment = () => {
        ...
        pendingRedirect = false;
        pendingRead = false;
        if (current.words.length > 0 || current.redirectTargets.length > 0)
...
function verbTargets(verb, args, dir) {
    ...
    switch (verb) {
        ...
        case "sed": {
            if (!sedInPlace)
                return []; // without -i, sed writes stdout only
...
export function checkBashWriteTarget(command, root = resolveGateRoot(), cwd = process.cwd()) {
    ...
    for (const segment of parseBashSegments(command)) {
        ...
        for (const target of segment.redirectTargets) {
            if (isCodePathAbs(resolveToken(dir, target), root))
                hits.push(target);
        }
...
function blockReason(toolName, targets, root) {
    ...
    return (`implementation-gate: blocked ${toolName} targeting code path ${targetsText}: this window holds no active ` +
...
export function gateDecision(toolName, input, env, cwd = process.cwd()) {
    ...
    if (toolName === "bash") {
        ...
        if (targets.length === 0)
            return { blocked: false, basis: "no-code-path-target" };
    }
    else {
        ...
        if (!isCodePath(rawPath, root, cwd))
            return { blocked: false, basis: "not-a-code-path" };
        targets = [rawPath];
    }
...
export function recordImplGateAudit(tag, toolName, target, basis, root = resolveGateRoot()) {
    try {
        const dir = path.join(root, AGENTICDOC_DIR);
        fs.mkdirSync(dir, { recursive: true });
        fs.appendFileSync(path.join(dir, GATE_LOG_FILE), `[GATE] ${new Date().toISOString()} ${tag} tool=${toolName} target=${target} basis=${basis}\n`, "utf8");
    }
    catch {
...
export function registerImplementationGate(pi) {
...
//# sourceMappingURL=implementation-gate.js.map