...
const PROTECTED_CONFIG_FILES = ["auth.json", "models.json", "settings.json", "oauth.json"];
...
const GUARD_EXPLANATION = "~/.pi/agent/{auth,models,settings,oauth}.json are cross-window config shared by every live pi session " +
...
export function protectedConfigPaths(agentDir) {
...
export function isProtectedConfigPath(agentDir, rawPath, cwd = process.cwd()) {
    if (typeof rawPath !== "string" || rawPath === "")
        return false;
    const target = fold(path.normalize(path.resolve(cwd, expandTildePath(rawPath))));
    if (target === fold(path.normalize(agentDir)))
        return true;
...
function protectedReferences(scan, agentDir) {
    ...
    const refs = [];
    if (fragmentPresent(scan, homeFrag))
        refs.push(homeFrag);
    if (dirFrag !== homeFrag && fragmentPresent(scan, dirFrag))
        refs.push(dirFrag);
...
export function checkProtectedBashCommand(agentDir, command) {
    ...
    if (refs.length === 0 && !(inline && bareFragment))
        return { prohibited: false };
    const constructs = [];
    ...
    for (const m of scan.matchAll(REDIRECT_RE)) {
        ...
        if (target !== "" && protectedReferences(target, agentDir).length > 0)
    ...
    return {
        prohibited: true,
        reason: `protected-config: blocked a bash command referencing cross-window config (${refText}) ` +
...
export function recordProtectedBlockTrace(toolName, detail, taskPathEnv = process.env.PI_WORKER_TASK) {
    if (!taskPathEnv)
        return;
    try {
        ...
        fs.appendFileSync(path.join(dir, "trace.log"), `[PROTECTED_CONFIG] ${new Date().toISOString()} blocked tool=${toolName} target=${first}\n`, "utf8");
    }
    catch {
...
export function registerProtectedConfigGuard(pi) {
