...
async function getLatestVersion(repo) {
    const headers = { "User-Agent": `${APP_NAME}-coding-agent` };
    if (process.env.GITHUB_TOKEN) {
        headers.Authorization = `Bearer ${process.env.GITHUB_TOKEN}`;
    }
    const response = await fetchWithRetry(`https://api.github.com/repos/${repo}/releases/latest`, { headers }, { timeoutMs: NETWORK_TIMEOUT_MS });
    if (!response.ok) {
...
async function downloadTool(tool, silent = false) {
    const config = TOOLS[tool];
    if (!config)
    ...
    try {
        ...
        version = pinned;
        if (!silent) {
            console.log(chalk.dim(`GitHub API unavailable (${e instanceof Error ? e.message : e}); using pinned ${config.name} ${pinned}`));
        }
    }
