...
describe("Baseten models", () => {
	it("keeps both GLM 5.2 endpoints multimodal", () => {
		expect(getModel("baseten", "zai-org/GLM-5.2").input).toEqual(["text", "image"]);
		expect(getModel("baseten", "zai-org/GLM-5.2-Fast").input).toEqual(["text", "image"]);
	});
